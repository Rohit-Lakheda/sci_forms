						...
						<div class="form-body">
							...
							<div class="tab-content">
								<div class="tab-pane active">
									...
									<div class="form-group" <?php //if($temp_booth_area == 4) {echo 'style="display:none;"';}?>>
										<label class="control-label col-md-3"> Is stall manning people same as delegate <span class="required"> * </span>
										</label>
										<div class="col-md-6">
											<select id="same-stall" name="same-stall" class="form-control" required="required" onchange="exhiShow(this);">
												<option value="">-- Select --</option>
												<option value="Yes">Yes</option>
												<option value="No" <?php //if($temp_booth_area == 4) {echo 'selected="selected"';}?>>No</option>
											</select>
										</div>
									</div>
									...
									<?php for($i_exb = 1; $i_exb<=$total_exbhitors;$i_exb++) { ?>
										<div class="h4">Personal Information of Exhibitor:
											<?php if($total_exbhitors > 1) {	
												 ...
												 if($i_exb<=2){?><span class="dips-required"> * </span><?php } ?>	
										</div>
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												<input type="number" class="form-control" name="mob<?php echo $i_exb;?>" id="mob<?php echo $i_exb;?>" maxlength="249" <?php if($i_exb<=2){?>required<?php } ?> />
											</div> 
										</div> 
										...
										<div class="form-group">
											...
											<div class="col-md-6">
												...
												<?php /* ?><select id="del_category<?php echo $i_exb;?>" name="del_category<?php echo $i_exb;?>" class="form-control" <?php if($i_exb<=2){?>required<?php } ?>>
									...
									<span class="dele-dpan" style="display: none;">
									<?php  
										for($i=1;$i<=$total_delegates;$i++) { ?>
										...
										<div class="form-group">
									    	<label class="control-label col-md-3">Name <span class="dips-required"> * </span></label>
									      	<div class="col-md-2">
									      		<select class="form-control" name="dtitle<?php echo $i; ?>" id="dtitle<?php echo $i; ?>">
										   			<option value="">-Title-</option>
													<?php $titleList = array('Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Prof.');
							                            foreach ($titleList as $title) {
							                            	$selected = '';
									      	...
									      	<div class="col-md-2"><input type="text" class="form-control" placeholder="First Name" name="dfname<?php echo $i; ?>" type="text" id="dfname<?php echo $i; ?>" maxlength="100" value="<?php if(isset($_SESSION['dfname'.$i])) { echo $_SESSION['dfname'.$i]; }else{echo @$qr_gt_user_data_ans_row['fname'.$i]; } ?>" ></div>
									      	<div class="col-md-2"><input type="text" class="form-control" placeholder="Last Name" name="dlname<?php echo $i; ?>" type="text" id="dlname<?php echo $i; ?>" maxlength="100" value="<?php if(isset($_SESSION['dlname'.$i])) { echo $_SESSION['dlname'.$i]; }else{ echo @$qr_gt_user_data_ans_row['lname'.$i]; } ?>"></div>
										    </div>
                                            <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-6">
                                                	<input class="form-control" name="badge<?php echo $i; ?>" type="text" id="badge<?php echo $i; ?>" maxlength="150" value="<?php if(isset($_SESSION['badge'.$i])) { echo $_SESSION['badge'.$i]; }else{ echo @$qr_gt_user_data_ans_row['badge'.$i]; } ?>" onkeyup="check_char(event,'badge<?php echo $i; ?>')"/>
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="col-md-3 control-label">Email Address <span class="dips-required"> * </span></label>
                                                <div class="col-md-6">
                                                	<input class="form-control" name="email_m<?php echo $i; ?>" type="email" id="email_m<?php echo $i; ?>" maxlength="150" value="<?php if(isset($_SESSION['email'.$i])) { echo $_SESSION['email'.$i]; }else{ echo @$qr_gt_user_data_ans_row['email'.$i]; } ?>" />
                                                </div>
                                            </div>
                                            <div class="form-group">
												...
												?>
												<input class="form-control" name="cellno<?php echo $i; ?>" type="number" id="cellno<?php echo $i; ?>" maxlength="10"  value="<?php echo $mobile;?>" onkeyup="check_num(event, 'cellno<?php echo $i; ?>');"/>
                                                </div>
                                            </div>
                                            <div class="form-group">
											<label class="control-label col-md-3"> Selected Category
									...
									<?php	
											$EXT = '414';
											if($temp_exhi_sector == 'Bio Technology') {
													$EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_NAME = 'Leela Malladi';
													$EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_EMAIL = 'leela.malladi@mmactiv.com';
													$EXT = '418';
												}
									 /*?>
									<div class="tab-content">
										<div class="tab-pane active">
											<div class="note note-success">
												<p>	If you want more Exhibitors please contact our executive <br />
													</span>Name: <?php echo $EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_NAME;?><br />
													Email: <a href="mailto:<?php echo $EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_EMAIL;?>"><?php echo $EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_EMAIL;?></a>
												<?php Mobile: <?php echo $EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_MOBILE_NO; ?><br />
													Phone: <?php echo $EVENT_DB_FORM_EXHIBITOR_DIR_CONTACT_PERSON_PHONE_NO. '  Extn.'.$EXT;?>
												</p>
											</div>
										</div>
									</div><?php */?>
