<?php
	...
	*/
	...
	for ($i = 1; $i <= $lmt; $i++) {
	    ...
	    $cata_day1 = @$_POST['cat1' . $i];
	    $cata_day2 = @$_POST['cat2' . $i];
	    $cata_day3 = @$_POST['cat3' . $i];
	    if (($cata_day1 == "") && ($cata_day2 == "") && ($cata_day3 == "")) {
	        echo "<script language='javascript'>alert('Please Enter Complete Information');</script>";
	        echo "<script language='javascript'>window.location = 'reg_registrations5.php';</script>";
	        exit;
	    }
	    $cata_dele = '';
	    if (!empty($cata_day1) && !empty($cata_day2) && !empty($cata_day3)) {
	        $cata_dele = $cata_day1 . '#' . $cata_day2 . '#' . $cata_day3;
	        
	        $amt_payable = 2500;
	        $tax = round(($amt_payable * 14.5) / 100);	// calculating tax
	        $total_amt_payable = $amt_payable + $tax;
	        
	    } else if (!empty($cata_day1) && !empty($cata_day2)) {
	        $cata_dele = $cata_day1 . '#' . $cata_day2;
	        
	        $amt_payable = 2000;
	        $tax = round(($amt_payable * 14.5) / 100);	// calculating tax
	        $total_amt_payable = $amt_payable + $tax;
	    } else if (!empty($cata_day2) && !empty($cata_day3)) {
	        $cata_dele = $cata_day2 . '#' . $cata_day3;
	        
	        $amt_payable = 2000;
	        $tax = round(($amt_payable * 14.5) / 100);	// calculating tax
	        $total_amt_payable = $amt_payable + $tax;
	    } else if (!empty($cata_day1) && !empty($cata_day3)) {
	        $cata_dele = $cata_day1 . '#' . $cata_day3;
	    ...
	    if (($res['cata'] != "Exhibitors Delegate")) {
	        /*
	        ...
	        echo "<script language='javascript'>window.location='nano_registrations5.php';</script>";
	        exit;
	        }
	        */
	    }
	    ...
	    $total_amt_per_dele = $total_amt_per_dele + $total_amt_payable;
	    $selection_amt += $amt_payable;
	    $taxTotal += $tax;
	    ...
	    mysqli_query($link,"UPDATE " . $EVENT_DB_FORM_REG_DEMO . " SET $a9 = '$cata_dele' where reg_id = '$reg_id'") or die(mysqli_error($link));
	    mysqli_query($link,"UPDATE " . $EVENT_DB_FORM_REG_DEMO . " SET $a10 = '$amt_payable' where reg_id = '$reg_id'") or die(mysqli_error($link));
	    mysqli_query($link,"UPDATE " . $EVENT_DB_FORM_REG_DEMO . " SET selection_amt = '$selection_amt' where reg_id = '$reg_id'") or die(mysqli_error($link));
	    
	    mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_DEMO." SET tax = '$taxTotal' WHERE reg_id = '$reg_id'")or die(mysqli_error($link));
	    mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_DEMO." SET total = '$total_amt_per_dele' WHERE reg_id = '$reg_id'")or die(mysqli_error($link));
	...
	echo "<script language='javascript'>window.location = 'reg_registrations7.php';</script>";
	exit;
?>