...
if (isset($_POST['make_payment'])) {
	...
	if (!empty($qr_gt_user_data_ans_row)) {
		$qr_gt_user_data_ans_row['delegate_flag'] = $_POST['delegate_flag'];
		$fields = '';
		$values = '';
	...
	if ($res['sector'] == 'Bio Technology') {
		$recipient[] = '';
		$recipient = array('test.interlinks@gmail.com', 'vani.faustina@mmactiv.com', 'ambika.kiran@mmactiv.com');
	}
	...
	$mail->SetFrom($EVENT_ENQUIRY_EMAIL, $EVENT_NAME . ' ' . $EVENT_YEAR);
	$mail->Subject = "Thank you for Initiating Registration with " . $EVENT_NAME . " " . $EVENT_YEAR;
	$mail->MsgHTML($mail_body);
	...
	$recipients = array('test.interlinks@gmail.com', '', 'vani.faustina@mmactiv.com');
	elastic_mail("Thank you for Initiating Registration with " . $EVENT_NAME . " " . $EVENT_YEAR, $mail_body, $recipients);
	//echo $mail_body;exit;
	//exit;
