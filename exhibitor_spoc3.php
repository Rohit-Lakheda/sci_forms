<?php
	...
	echo "<script language='javascript'>window.location= 'exhibitor_spoc4.php?rt=retds4fn324rn_ed24d3it&assoc_nm=$assoc_nm';</script>";
	exit ();
	...
	//if($_POST['same-stall'] == 'No') {
		...
		for($i=1; $i<= $lmt; $i++) {
			...
			if(!empty($email)) {
				...
				$fname = mysqli_real_escape_string($link,@$_POST['dfname'.$i]);
				$fname = trim($fname);
			
				$lname = mysqli_real_escape_string($link,@$_POST['dlname'.$i]);
				$lname = trim($lname);
			
				$job_title = @$_POST['job_title'.$i];
				$job_title = trim($job_title);
			
				$badge = mysqli_real_escape_string($link,@$_POST['badge'.$i]);
				$badge = trim($badge);
			
				
			
				$cellno = $ccode . $_POST['cellno'.$i];
				//$cata = @$_POST['cata'.$i];
			
				$amt = $amt+$rate_org;
			
				$pas1=str_replace("'", '', $_POST['dfname'.$i]) . "123";
				$pas2=md5($pas1);
