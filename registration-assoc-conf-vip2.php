...
$_SESSION['email1'] = $_POST['email_m1'];
$_SESSION['foneCountryCode'] = $_POST['foneCountryCode'];
$_SESSION['cellno1'] = $_POST['cellno1'];
$_SESSION['job_title1'] = $_POST['job_title1'];
...
for ($j = 1; $j <= $lmt; $j++) {
	...
	if ($num_row > 0) {
		...
		echo "<script language='javascript'>window.location = ('registration-assoc-conf-vip.php?ret=retds4fu324rn_ed24d3it&en=" . $en . "&a=" . $assoc_code . "');</script>";
		exit;
	}
}
...
if (($city == "") || ($country == "")) {
	...
	echo "<script language='javascript'>window.location = 'registration-assoc-conf-vip.php?assoc_name=$assoc_name';</script>";
	exit;
}
...
/*for ($j = 1; $j <= $lmt; $j++) {
	...
	$num_row = mysqli_num_rows($qr);
	if ($num_row > 0) {
		echo "<script language='javascript'>alert('Entered email already registered.');</script>";
		echo "<script language='javascript'>window.location = 'registration-assoc-conf-vip.php?assoc_name=$assoc_name';</script>";
		exit;
	}
}*/
...
if($resulre['registration_category'] == "GIA Partner"){
	$org = $org . ", " . $resulre['assoc_name'];
}
...
echo "<script language='javascript'>window.location = 'registration-assoc-conf-vip7.php?a=$assoc_code';</script>";
exit;
