...
$temp_booth_no = mysqli_escape_string($link,htmlspecialchars(@$_POST['booth_no']));
$temp_booth_area = mysqli_escape_string($link,htmlspecialchars(@$_POST['booth_area']));
...
$temp_booth_area_unit = mysqli_escape_string($link,htmlspecialchars(@$_POST['booth_area_unit']));
$temp_fascia_name = mysqli_escape_string($link,htmlspecialchars(@$_POST['fascia_name']));
...
$temp_fax_cntry = mysqli_escape_string($link,htmlspecialchars(@$_POST['faxCountryCode']));
// $temp_fax_area = @$_POST['fax_area'];
$temp_fax = mysqli_escape_string($link,htmlspecialchars(@$_POST['fax']));
...
$temp_profile = mysqli_escape_string($link,htmlspecialchars(@$_POST['exbhi_profile']));
$temp_profile = mysqli_escape_string($link,htmlspecialchars($temp_profile));
$category = mysqli_escape_string($link,htmlspecialchars(@$_POST['category']));
$exhi_profile = '';
if (isset($_POST['exhi_profile'][0])) {
	//$exhi_profile = implode('#', $_POST['exhi_profile']);
	$exhi_profile = mysqli_escape_string($link,htmlspecialchars(@$_POST['exhi_profile'][0]));
	if ($exhi_profile == 'Other') {
		$exhi_profile .= ' - ' . mysqli_escape_string($link,htmlspecialchars(@$_POST['exhi_profile-other']));
	}
}
...
$_SESSION['assoc_nm'] = $assoc_nm = mysqli_escape_string($link,htmlspecialchars(@$_POST['assoc_nm']));
...
if (!empty($assoc_nm)) {
	...
	else if (($temp_booth_area == 1) || ($temp_booth_area == 2) || ($temp_booth_area == 3) || ($temp_booth_area == 4) || ($temp_booth_area == 6 || $temp_booth_area = "Startup Booth")) {
		$total_exbhitors = 2;
		$temp_total_exbhitors = $total_exbhitors;
		$total_del = 1;
		$temp_total_del = $total_del;
	...
	if (($temp_booth_area >= 3) && ($temp_booth_area <= 8)) {
		...
		$total_del = 7;
		$temp_total_del = $total_del;
	} else if (($temp_booth_area >= 82) && ($temp_booth_area <= 135)) {
		$total_exbhitors = 7;
		$temp_total_exbhitors = $total_exbhitors;
		$total_del = 7;
		$temp_total_del = $total_del;
...
if (!empty($assoc_nm)) {
	...
	if (($temp_booth_area <= 1 && $temp_booth_area != "Startup Booth")) {
		echo "<script language='javascript'>alert('Booth/ Stall area should be greater than or equal to 1 sqm');</script>";
		mysqli_close($link);
		echo "<script language='javascript'>window.location = ('exhibitor.php?rt=retds4fn324rn_ed24d3it&assoc_nm=$assoc_nm');</script>";
		exit();
	}
	...
	<div class="col-md-12">
		<div class="portlet light bordered" id="registration_form_2">
			...
			<div class="portlet-body form">
				<form action="exhibitor3.php?ret=<?php echo $return; ?>&assoc_nm=<?php echo $assoc_nm; ?>"
					class="form-horizontal" name="reg_registration_form_2" id="reg_registration_form_2" method="post"
					onsubmit="return validate_ex2();">
						...
						<div class="form-body">
							...
							<div class="tab-content">
								<div class="tab-pane active">
									...
									<div class="alert alert-info">Please provide the <strong>Names of the persons who
											would be manning the stall (EXHIBITOR)</strong> from your organisation and
										<strong>Names of persons who would be attending the sessions (DELEGATE)</strong>
										by selecting the appropriate drop down below:
									</div>
									<input type="hidden" name="speaker_event_name" id="speaker_event_name"
										value="<?php echo $EVENT_NAME; ?>" />
									<input type="hidden" name="speaker_event_year" id="speaker_event_year"
										value="<?php echo $EVENT_YEAR; ?>" />
									...
									<?php for ($i_exb = 1; $i_exb <= $total_exbhitors; $i_exb++) { ?>
										<div class="h4">Personal Information of Exhibitor:
											...
											<input name="del_category<?php echo $i_exb; ?>" type="hidden"
												id="del_category<?php echo $i_exb; ?>" class="del_category"
												value="Exhibitor" />
										</div>
										<div class="form-group">
											<label class="control-label col-md-3">Contact Person
												Name<?php if ($i_exb <= 2) { ?> <span class="required"> *
													</span><?php } ?></label>
											<div class="col-md-2">
											...
											<div class="col-md-2"><input type="text" class="form-control"
													placeholder="First Name" name="fname<?php echo $i_exb; ?>"
													id="fname<?php echo $i_exb; ?>" <?php if ($i_exb <= 2) { ?>required<?php } ?> maxlength="100" onkeyup="check_char(event,'cp_fname');"></div>
											<div class="col-md-2"><input type="text" class="form-control"
													placeholder="Last Name" name="lname<?php echo $i_exb; ?>"
													id="lname<?php echo $i_exb; ?>" <?php if ($i_exb <= 2) { ?>required<?php } ?> maxlength="100" onkeyup="check_char(event,'cp_lname');"></div>
										</div>
										<div class="form-group">
											<label class="col-md-3 control-label">Designation <?php if ($i_exb <= 2) { ?>
													<span class="dips-required"> * </span> <?php } ?></label>
											<div class="col-md-6">
												<input type="text" class="form-control" name="desig<?php echo $i_exb; ?>"
													id="desig<?php echo $i_exb; ?>" maxlength="249" <?php if ($i_exb <= 2) { ?>required<?php } ?> />
											</div>
										</div>
										<div class="form-group">
											<label class="col-md-3 control-label">Mobile No <?php if ($i_exb <= 2) { ?>
													<span class="dips-required"> * </span> <?php } ?></label>
											<div class="col-md-6">
												<input type="number" class="form-control" name="mob<?php echo $i_exb; ?>"
													id="mob<?php echo $i_exb; ?>" maxlength="12" <?php if ($i_exb <= 2) { ?>required<?php } ?> />
											</div>
										</div>
										...
										<div class="form-group">
											<label class="col-md-3 control-label">Email Address <?php if ($i_exb <= 2) { ?>
													<span class="dips-required"> * </span> <?php } ?></label>
											<div class="col-md-6">
												<input type="email" class="form-control" name="email<?php echo $i_exb; ?>"
													id="email<?php echo $i_exb; ?>" <?php if ($i_exb <= 2) { ?>required<?php } ?> />
											</div>
										</div>
									<?php } ?>
									...
									<span class="dele-dpan" style="display: none1;">
										...
										for ($i = 1; $i <= $total_del; $i++) { ?>
											...
											<div class="form-group">
												<label class="control-label col-md-3">Name <?php if ($i <= 2) { ?><span
															class="dips-required"> * </span><?php } ?></label>
												<div class="col-md-2">
													<select class="form-control" name="dtitle<?php echo $i; ?>"
														id="dtitle<?php echo $i; ?>" <?php if ($i <= 2) { ?>required<?php } ?>>
														<option value="">-Title-</option>
														<?php $titleList = array('Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Prof.');
														foreach ($titleList as $title) {
															$selected = '';
												...
												<div class="col-md-2"><input type="text" class="form-control"
														placeholder="First Name" name="dfname<?php echo $i; ?>" type="text"
														id="dfname<?php echo $i; ?>" maxlength="255" value="<?php if (isset($_SESSION['dfname' . $i])) {
															   echo $_SESSION['dfname' . $i];
														   } else {
															   echo @$qr_gt_user_data_ans_row['fname' . $i];
														   } ?>" <?php if ($i <= 2) { ?>required<?php } ?>></div>
												<div class="col-md-2"><input type="text" class="form-control"
														placeholder="Last Name" name="dlname<?php echo $i; ?>" type="text"
														id="dlname<?php echo $i; ?>" maxlength="255" value="<?php if (isset($_SESSION['dlname' . $i])) {
											...
											<div class="form-group">
												<label
													class="col-md-3 control-label">Designation<?php if ($i <= 2) { ?><span
															class="dips-required"> * </span><?php } ?></label>
												<div class="col-md-6">
													<input class="form-control" name="job_title<?php echo $i; ?>"
														type="text" id="job_title<?php echo $i; ?>" maxlength="100" value="<?php if (isset($_SESSION['job_title' . $i])) {
											...
											<div class="form-group">
												<label class="col-md-3 control-label">Name on Badge
													<?php if ($i <= 2) { ?><span class="dips-required"> *
														</span><?php } ?></label>
												<div class="col-md-6">
													<input class="form-control" name="badge<?php echo $i; ?>" type="text"
														id="badge<?php echo $i; ?>" maxlength="150" value="<?php if (isset($_SESSION['badge' . $i])) {
											...
											<div class="form-group">
												<label class="col-md-3 control-label">Mobile Number
													<?php if ($i <= 2) { ?><span class="dips-required"> *
														</span><?php } ?></label>
												<div class="col-md-6">
													<?php
													...
													?>
													<input class="form-control" name="cellno<?php echo $i; ?>" type="number"
														id="cellno<?php echo $i; ?>" maxlength="12"
														value="<?php echo $mobile; ?>"
														onkeyup="check_num(event, 'cellno<?php echo $i; ?>');" <?php if ($i <= 2) { ?>required<?php } ?> />
												</div>
											</div>
											<?php /*?><div class="form-group">
																			 <label class="control-label col-md-3"> Selected Category
												...
												<input name="catagory<?php echo $i; ?>" type="hidden"
													id="catagory<?php echo $i; ?>" value="Complimentary Exhibitors Delegate" />
											<?php } else { ?>
												<input name="catagory<?php echo $i; ?>" type="hidden"
													id="catagory<?php echo $i; ?>" value="Complimentary Sponsor Delegate" />
											<?php } ?>
										<?php } ?>
										<?php if ($category == 'Exhibitor') { ?>
									...
									<div class="form-group">
										<div class="col-md-6 col-md-offset-3">
											<div class="note note-success">
												...
												<p> If you have any technical problem, please contact our executive
													<br />
													</span>Name: <?php echo $EVENT_DB_FORM_TECHNICAL_PERSON_NAME; ?>
													<br />
													Email: <a
														href="mailto:<?php echo $EVENT_DB_FORM_TECHNICAL_PERSON_EMAIL; ?>"><?php echo $EVENT_DB_FORM_TECHNICAL_PERSON_EMAIL; ?></a>
													<br />
