<?php
	...
	if(isset($_POST['make_payment'])) {
		...
		if($qr_gt_user_data_ans_row['org_reg_type'] == 'Premium Delegate') {
		...
		while ($qr_gt_user_inx_login_data_ans = mysqli_fetch_array($qr_gt_user_inx_login_data_id)) {			
			...
			$recipients = array($qr_gt_user_inx_login_data_ans['pri_email'], '', 'test.interlinks@gmail.com', '', $EVENT_ENQUIRY_EMAIL, '', '', '', '');
			/*$mail = new PHPMailer();
		...
		//$recipient = array('','test.interlinks@gmail.com','', 'sagar.patil@interlinks.in');
	...
	echo "<script language='javascript'>window.location = 'registration_comp9.php?id=" . $qr_gt_user_data_ans_row['tin_no'] . "';</script>";
	exit;
?>
