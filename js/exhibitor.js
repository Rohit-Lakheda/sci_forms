...
tinyMCE.init({
		...
		temptext = temptext.replace(/&nbsp;/gi, "");
	     tinylen = tinymax - temptext.length;
		 //alert(tinylen+'auto');
		 ...
		 if (tinymax < temptext.length) {
			/*e.preventDefault();
			e.stopPropagation();
			return false;*/
			$('#limit-char').html('<span style="color:red">' + htmlcount + '</span>');
		 } else {
			// prevent insertion of typed character
			 $('#limit-char').html(htmlcount);
		 }
		 $('#charlimit').val(tinylen);
		 /*if (tinymax < tinylen) {
			 	//e.preventDefault();
		        //e.stopPropagation();
		        //return false;
...
function validate_ex() {
	...
	var assoc_nm = '';
	if($('#assoc_nm').length) {
		var assoc_nm = $('#assoc_nm').val();
	}
	if(assoc_nm == '') {
		if (chk_booth_area <= 8) {
			alert("Booth/ Stall area should be greater or equal to 9sqm");
			document.getElementById("booth_area").focus();
			return false;
		}
	}
...
function profileTextCount() {
	...
	prof_len = prof_len.replace(/&nbsp;/gi, "");
	var tinymax = 780;
	proftinylen = tinymax - prof_len.length;
	//alert(proftinylen);
	htmlcount = "Character Count: " + proftinylen;
	//alert(htmlcount);
	
	if (tinymax < prof_len.length) {
		/*e.preventDefault();
		e.stopPropagation();
		return false;*/
		$('#limit-char').html('<span style="color:red">' + htmlcount + '</span>');
