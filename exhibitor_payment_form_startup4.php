<?php
	...
	if ((!isset($_SESSION["vercode_ex"])) || ($_SESSION["vercode_ex"] == '')) {
		...
		echo "<script language='javascript'>alert('Verification images mis-matched.');</script>";
			echo "<script language='javascript'>window.location = 'exhibitor_payment_form_startup.php';</script>";
		exit;
	}
	...
	if(isset($_POST['make_payment'])) {
		...
		$dele_title      = $res['cp_title'];
		$dele_fname      = $res['cp_fname'];
		$dele_lname      = $res['cp_lname'];
		$dele_email      = $res['cp_email'];
		$job_title       = '';//$res['job_title'];
		$dele_cellno     = $res['cp_mobile'];
		...
		$data['mobile']= $phone;
		$data['company']= $res['exhibitor_name'];
		...
		//========================== Interlinx
		if($qr_gt_user_data_ans_row1['cata2'] == 'Premium Delegate' || $qr_gt_user_data_ans_row1['cata2'] == 'International Premium Delegate') {
		...
		/*$qr_gt_user_data_id2      = mysqli_query($link,"SELECT * FROM " . $EVENT_DB_FORM_INTERLINX_REG_TBL_DEMO . " WHERE reg_id = 'ex$reg_id'");
		$qr_gt_user_data_ans_row2 = mysqli_fetch_assoc($qr_gt_user_data_id2);
		if(!empty($qr_gt_user_data_ans_row2)) {
			$qry_email_chk       = mysqli_query($link,"SELECT * FROM " . $EVENT_DB_FORM_INTERLINX_REG_TBL . " WHERE pri_email = '" . $qr_gt_user_data_ans_row2['pri_email'] . "'");
			$qry_email_chk_num   = mysqli_num_rows($qry_email_chk);
	        ...
	        if ($qry_email_chk_num >= 1) {
				...
				$fields = '';
				$values = '';
				foreach ($qr_gt_user_data_ans_row2 as $key=>$value) {
					if($key != 'srno') {
						$fields .= $key . ',';
				...
				$test_title = $qr_gt_user_data_ans_row2['title'];
				$test_fname = $qr_gt_user_data_ans_row2['fname'];
				$test_lname = $qr_gt_user_data_ans_row2['lname'];
				$test_email = $qr_gt_user_data_ans_row2['pri_email'];
				$usr_no1 = $qr_gt_user_data_ans_row2['user_id'];
				//------------------------------------------------- Inserting Values in interlinx registration table --------------------------------------
		...
		//exit;
		echo "<script language='javascript'>window.location = 'exhibitor_payment_form_startup5.php?id=" . $qr_gt_user_data_ans_row['tin_no'] . "';</script>";
		exit;
