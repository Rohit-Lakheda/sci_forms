...
if (isset($_POST['make_payment'])) {
	...
	if (!empty($qr_gt_user_data_ans_row)) {
		...
		foreach ($qr_gt_user_data_ans_row as $key => $value) {
			...
			if ($key != 'srno') {
				...
				$fields .= mysqli_escape_string($link, htmlspecialchars($key)) . ',';
	...
	$recipients = array('test.interlinks@gmail.com', $EVENT_ENQUIRY_EMAIL, 'sanjeevbrao@gmail.com', 'sathyakanth.manukar@mmactiv.com');
	...
	// echo "<script language='javascript'>window.location = ('exhibitor_payment_form5_no_pg.php?id=" . $qr_gt_user_data_ans_row['tin_no'] . "');</script>";
	...
	/*
			 ...
			 $qrygetdata_ans = mysql_fetch_array($qrygetdata);
