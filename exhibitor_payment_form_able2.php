...
$_SESSION['user_type'] = $assoc_nm = $user_type = @$_POST['user_type'];
...
if ($user_type == 'ABLE StartUp Exhibitor') {
...
if (($temp_exhi_name == "") || ($temp_addr1 == "") || ($temp_city == "")  || ($temp_state == "")  || ($temp_zip == "") || ($temp_email == "")) {
	...
	echo "<script language='javascript'>window.location = 'exhibitor_payment_form_able.php?rt=retds4fn324rn_ed24d3it';</script>";
	exit;
}
...
if ($booth_size == '9 SQM') {
	$selection_amt = 91800;
} else if ($booth_size == '18 SQM') {
	$selection_amt = 183600;
} else if ($booth_size == '36 SQM') {
	$selection_amt = 336600;
}
...
/*mysqli_query($link,"insert into ".$EVENT_DB_FORM_REG_DEMO."(org_reg_type,nationality,curr,gr_type,sub_delegates,paymode,pay_status,amt_ext,reg_id,reg_date,reg_time,sector,tin_no,pin_no, cata) values 
    ...
    */
echo "<script language='javascript'>window.location = 'exhibitor_payment_form_able3.php';</script>";
exit;
