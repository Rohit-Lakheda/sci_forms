<?php
    ...
    if (empty($_SESSION["vercode_reg"]) || mysqli_real_escape_string($link,htmlspecialchars(($_POST["vercode"])) != $_SESSION["vercode_reg"])) {
		...
		if(!empty($assoc_name)) {
			echo "<script language='javascript'>window.location = 'registration-walk.php?en=$en&assoc_name=$assoc_name';</script>";
		} else {
			echo "<script language='javascript'>window.location = 'registration-walk.php?en=$en';</script>";
		}
        exit;
    }
	...
	if ($ret == "retds4fu324rn_ed24d3it") {
	    mysqli_query($link,"delete from " . $EVENT_DB_FORM_REG_WALK_DEMO . " where reg_id = '$reg_id' ") or die(mysqli_error($link));
	}
	...
	if(!empty($assoc_srno) && !empty($user_type)) {
		...
		if(mysqli_num_rows($resulre) <= 0) {
			...
			echo "<script language='javascript'>window.location='registration-walk.php';</script>";
			exit;
		}
	...
	if( ($org == "")){
	    ...
	    echo "<script language='javascript'>window.location = 'registration-walk.php?assoc_name=$assoc_name';</script>";
	    exit;
	}
	...
	mysqli_query($link,"insert into " . $EVENT_DB_FORM_REG_WALK_DEMO . "(org_reg_type,nationality,cata,curr,gr_type,sub_delegates,paymode,
	...
	user_type,conference_type) values ('$cata_m','$nationality','$cata','$curr','$grp','$total_dele','$paymode','$pay_status','$amt_ext','$rate_org','$amt','$amt','$reg_id','$ddate','$ttime','$temp_sess_sp_msg','$dollar','$assoc_name', '$member_of_assoc','$assoc_srno','$user_type','$conf_type')") or die(mysqli_error($link));
	
	$qry = mysqli_query($link,"SELECT * FROM " . $EVENT_DB_FORM_REG_WALK_DEMO . " WHERE reg_id = '$reg_id'") or die(mysqli_error($link));
	$res = mysqli_fetch_array($qry);

	$fname = mysqli_real_escape_string($link,htmlspecialchars($_POST['fname1']));
	$lname = mysqli_real_escape_string($link,htmlspecialchars($_POST['lname1']));
	$badge = $fname . " " . $lname;
	$selection_amt = 1000;
	$tax = 180;
	 $processing_charge_per = 3; //3%
	
	 $processing_charge = ($selection_amt * $processing_charge_per) / 100;
	
	 $total_amount = $processing_charge + $selection_amt+ $tax;
	
	 $total_fee = $total_fee + $selection_amt;
	
	
	//update table with fname1,lname1,email1,  jobtitle1, cellno1, 
	mysqli_query($link,"UPDATE " . $EVENT_DB_FORM_REG_WALK_DEMO . " SET amt_ext='Rs.',pay_status='Not Paid', nationality = 'Indian Organization',paymode = 'Online', amt1 = '$selection_amt',processing_charge='$processing_charge', badge1 = '$badge',cata1 = 'Bengaluru Walks Attendee',processing_charge_per = '$processing_charge_per', selection_amt = '$selection_amt', tax='$tax',total='$total_amount' , fname1 = '" . mysqli_real_escape_string($link,htmlspecialchars($_POST['fname1'])) . "', lname1 = '" . mysqli_real_escape_string($link,htmlspecialchars($_POST['lname1'])) . "', email1 = '" . mysqli_real_escape_string($link,htmlspecialchars($_POST['email1'])) . "', job_title1 = '" . mysqli_real_escape_string($link,htmlspecialchars($_POST['job_title1'])) . "', cellno1 = '" . mysqli_real_escape_string($link,htmlspecialchars($_POST['cellno1'])) . "' WHERE reg_id = '$reg_id'") or die(mysqli_error($link));
	...
	do {
	    ...
	    $qry    = mysqli_query($link,"SELECT * FROM " . $EVENT_DB_FORM_REG_WALK_DEMO . " WHERE tin_no = '$tin_no1'");
	    $res_no = mysqli_num_rows($qry);
	    
	    $qry1    = mysqli_query($link,"SELECT * FROM " . $EVENT_DB_FORM_REG_WALK . " WHERE tin_no = '$tin_no1'");
	...
	//------------------------------------------ End Geneating Tin Number ----------------------------------------------------
	mysqli_query($link,"UPDATE " . $EVENT_DB_FORM_REG_WALK_DEMO . " SET tin_no = '$tin_no1' WHERE reg_id = '$reg_id'") or die(mysqli_error($link));
	...
	/* if(isset($_POST['daytype']) && !empty($_POST['daytype'])) {
		...
		if(!empty($offer)) {
			mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_WALK_DEMO." SET user_type = '$offer' WHERE reg_id = '$reg_id'")or die(mysqli_error($link));
		}
	} */
	
	mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_WALK_DEMO." SET event_name = '$event_name', sector = '$sector' WHERE reg_id = '$reg_id'")or die(mysqli_error($link));
	
	mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_WALK_DEMO." SET event_know = '$event_know' WHERE reg_id = '$reg_id'")or die(mysqli_error($link));
	...
	//exit;
	//echo "UPDATE ".$EVENT_DB_FORM_REG_WALK_DEMO." SET org='$org',addr1='$addr1',addr2='$addr2',city='$city',state='$state',country='$country',pin='$pin',fone='$fone',fax='$fax' WHERE reg_id = '$reg_id' ";
	mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_WALK_DEMO." SET org='$org',nature='$nature',addr1='$addr1',addr2='$addr2',city='$city',state='$state',country='$country',pin='$pin',fone='$fone',fax='$fax', gst_number='$gst_number',
	...
	echo "<script language='javascript'>window.location = 'walk-registration7.php?en=$en&assoc_name=$assoc_name';</script>";
	exit;
?>