...
for ($i = 1; $i <= $lmt; $i++) {
	...
	if ($curr == "Indian") {
		...
		if ($cata == 'Premium Delegate') {
			...
			if (date('Y-m-d') >= '2024-02-27' && date('Y-m-d') <= '2024-10-29') {
				$rate = "3000";
				$rate_org = "3000";
			} else if (date('Y-m-d') >= '2024-10-30' && date('Y-m-d') <= '2024-11-17') {
				$rate = "5000";
				$rate_org = "5000";
		...
		if ($assoc_name == "SOFTWARE TECHNOLOGY PARKS OF INDIA(STPI)") {
			$rate = "3000";
			$rate_org = "3000";
