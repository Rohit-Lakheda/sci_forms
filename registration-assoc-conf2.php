<?php
	...
	$_SESSION['org'] = mysqli_real_escape_string($link,htmlspecialchars((@$_POST['org'])));
	...
	$assoc_name = mysqli_real_escape_string($link,htmlspecialchars(@$_POST['assoc_name']));
	$assoc_name = trim($assoc_name);
	
	$assoc_code = mysqli_real_escape_string($link,htmlspecialchars(@$_POST['promo_code']));
	$assoc_code = trim($assoc_code);
	
	$po_vercode = htmlspecialchars($_POST["vercode"]); 
	

    if (empty($_SESSION["vercode_reg"]) || ($po_vercode != $_SESSION["vercode_reg"])) {
        session_destroy();
	...
	$cata_m = mysqli_real_escape_string($link,htmlspecialchars(@$_POST['cata_m']));
	/* if ($cata_m == "") {
	    session_destroy();
	    echo "<script language='javascript'>alert('Please select delegate type.');</script>";
	    if(!empty($assoc_name)) {
	...
	//print_r($_POST);exit;
	$total_dele = htmlspecialchars($_POST['total_dele']);
	...
	if( ($org == "") || ($city == "") || ($country == "")  || (@$_POST['fone'] == "")){
	    ...
	    echo "<script language='javascript'>window.location = 'registration-assoc-conf.php?assoc_name=$assoc_name';</script>";
	    exit;
	}
	...
	if($_POST['daytype'] == 'Single Day') {
		...
		$days = '';
		if (mysqli_real_escape_string($link,htmlspecialchars($_POST['day1'])) != "") {
	...
	//------------------------------------------ End Geneating Tin Number ----------------------------------------------------
	$prn = str_replace('TIN', 'PRN', $tin_no1);
	mysqli_query($link,"UPDATE " . $EVENT_DB_FORM_REG_DEMO . " SET tin_no = '$tin_no1', pin_no='$prn' WHERE reg_id = '$reg_id'") or die(mysqli_error($link));
	...
	/*exit;
	//echo "<script language='javascript'>window.location = 'registration-conference3.php?en=$en&assoc_name=$assoc_name';</script>";
	//exit;*/
	$gst_number = '';
	if($gst=='Registered'){
	    $gst_number = mysqli_real_escape_string($link,htmlspecialchars($_POST['gst_number']));
