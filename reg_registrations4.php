...
//--------------------------------------------------- Start  checking exhibitor -----------------------------------------------------------------------	
	...
	if( ($qr_gt_user_data_ans_no<=0) || ($qr_gt_user_data_ans_no=="") ){
		...
		echo "<script language='javascript'>window.location=('reg_registrations.php?exhi=$exhi&exhibitor_id=$exhibitor_id');</script>";
		echo "<script language='javascript'>document.location=('reg_registrations.php?exhi=$exhi&exhibitor_id=$exhibitor_id');</script>";
		exit; 
	}	
	...
	if(($qr_chk_exbhi_num_rows == 0) || ($qr_chk_exbhi_num_rows == ""))
	{
		...
		$total_exbhitors = (round($temp_booth_area/9)*1);
		
		if(($temp_booth_area<9) && (($temp_booth_area>1)) ){
			session_destroy();
			echo "<script language='javascript'>alert('Please Contact Admin As Exhibitor booth area is below 9sqm..');</script>";
	...
	mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_DEMO." SET cp_name='$cp_name',cp_email='$cp_email',nature='$nature',org='$org',addr1='$addr1',addr2='$addr2',city='$city',state='$state',country='$country',pin='$pin',fone='$fone',fax='$fax' WHERE reg_id = '$reg_id' ") or die(mysqli_error($link));
