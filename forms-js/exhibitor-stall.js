...
function validate_ex() {
	...
	if (chk_booth_area < 3 || chk_booth_area > 200) {

		alert("Booth/ Stall area should be min greater or equal to 4 sqm and mix 200sqm");
		document.getElementById("booth_area").focus();
		return false;
	}
	...
	if (document.getElementById("exhi_profile").value == "") {
		alert("Please select Exhibitors Profile.");
...
function profileTextCount() {
	...
	proftinylen = prof_len.length;
	//alert(proftinylen);
	htmlcount = "Character Count: " + proftinylen + "/750";
	//alert(htmlcount);
	$('#limit-char').text(htmlcount);
}
