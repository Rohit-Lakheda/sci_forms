...
require 'db.php';
$EVENT_DB_FORM_INTERLINX_REG_TBL = 'it_2024_interlinx_reg_table';
$EVENT_BADGE_DATA_TBL = 'it_2024_badge_data';

$db_connection = $dbCon;


// Fetch data from it_2024_reg_tbl with paid or complimentary status
$sql = "SELECT * FROM it_2024_reg_tbl WHERE pay_status IN ('Paid', 'Complimentary', 'Free') AND srno BETWEEN 0 AND 10";
//$sql  = "select * from it_2024_reg_tbl where email1 = 'manishk_sharma@outlook.com'";
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    for ($i = 1; $i <= $lmt; $i++) {
        ...
        if ($qry_email_chk_num > 0) {
            ...
            $sql = "INSERT INTO   (name, email, cata, category_id, country_code, mobile, company, designation, country, tin_no, city) 
        VALUES ('$name', '$dele_email', '$reg_cata', '$cata_code', '$dele_cntry_code', '$dele_mob', '$org', '$dele_desig', '$country', '$tin_no1', '$city')";
            ...
            // Insert user details into the it_2024_badge_data table
...
{
    $sql = "UPDATE it_2024_reg_api_log SET response = ? WHERE booking_id = ?";
    $stmt = mysqli_prepare($db_connection, $sql);
