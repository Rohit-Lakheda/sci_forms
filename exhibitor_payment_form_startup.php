<?php //ini_set('display_errors', 1); ini_set('display_startup_errors', 1); error_reporting(E_ALL);
	...
	//$sql = "SELECT COUNT(sub_delegates) AS cn FROM it_2020_reg_tbl WHERE user_type='Startup cell Karnataka'";
	$sql = "SELECT COUNT(srno) AS cn FROM it_2020_exhibitors_dir_payment_tbl WHERE user_type='Startup cell Karnataka'";
	$res = mysqli_query($link,$sql);
	$data_res = mysqli_fetch_assoc($res);
	if(isset($data_res['cn'])) {
		if($data_res['cn'] >= 120) {
			$is_full = true;
		}
	}
	...
	<div class="col-md-12">
		<div class="portlet light bordered" id="registration_form_1">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-red bold uppercase">
						Exhibitor at Startup Zone: Certified startup under Karnataka Startup Cell
			...
			<div class="portlet-body form">
				<?php if(!$is_full) { //date('Y-m-d') <= '2020-11-19') {?>
				<form action="exhibitor_payment_form_startup2.php<?php echo !empty($ret) ? '?rt=' . $ret : ''; ?>" class="form-horizontal" name="exhibitors_form_1" id="exhibitors_form_1" method="post" onsubmit="return validate_ex();">
							...
							<div class="tab-content">
								<div class="tab-pane active">
									...
									<div class="main-form">
										...
										<h4>Please nominate Conference Delegate 1:</h4>
										<div class="form-group">
										...
										<h4>Please nominate Conference Delegate 2:</h4>
										<div class="form-group">
											<label class="control-label col-md-3">Delegate Name <span class="dips-required">  </span></label>
											<div class="col-md-2">
										...
										<h4>Please nominate B2B Partnering Delegate:</h4>
										<div class="form-group">
											<label class="col-md-3 control-label"></label>
											<div class="col-md-6">
												<label><input type="checkbox" id="sameas" onchange="addvalues();"/>Is B2B partnering delegate same as conference delegate</label>
											</div>
										</div>
										<div class="form-group">
				...
				<?php } else {?>
					<h1><?php echo $assoc_nm;?> Online Registrations seats are fulled for Startup cell Karnataka. <?php //for  echo $EVENT_NAME . ' ' . $EVENT_YEAR;  is closed.?></h1>
				<?php }?>
			</div>
		</div>
	</div>
  ...
  function show_cata() {
	...
	function addvalues() {
		if (document.getElementById("sameas").checked == true) {
			$('#part_title').val($('#del_title').val());
			$('#part_fname').val($('#del_fname').val());
			$('#part_lname').val($('#del_lname').val());
			$('#part_email').val($('#del_email').val());
			$('#part_mob').val($('#del_mob').val());
		}
	}
</script>
