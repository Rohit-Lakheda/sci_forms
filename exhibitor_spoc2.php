<?php 
	...
	if ($temp_profile_len > 751) {
		...
		echo "<script language='javascript'>window.location = ('exhibitor_spoc.php?rt=retds4fn324rn_ed24d3it&assoc_nm=$assoc_nm');</script>";
		exit ();
	}
	...
	if(!empty($assoc_nm)) {
		...
		if (($temp_booth_area <= 9) && ($temp_booth_area >= 4)) {
			...
			$total_del = 0;//(floor ( $temp_booth_area / 9 ) * 1);
			$temp_total_del = $total_del;
			$total_del_max_flag = "False";
		}
	}
	...
	if(!file_exists($logo)) {
		echo "<script language='javascript'>alert('There was an error uploading your logo.');</script>";
		echo "<script language='javascript'>window.location = ('exhibitor_spoc.php?rt=retds4fn324rn_ed24d3it');</script>";
		exit ();
	}*/
?>
			...
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject font-red bold uppercase"> 
						Exhibitor Personnel Form
					 </span>
				</div>
			</div>
			<div class="portlet-body form">
				<form action="exhibitor_spoc3.php?ret=<?php echo $return; ?>&assoc_nm=<?php echo $assoc_nm; ?>" class="form-horizontal" name="reg_registration_form_2" id="reg_registration_form_2" method="post" onsubmit="return validate_ex2();">
						...
						<div class="form-body">
							...
							<div class="tab-content">
								<div class="tab-pane active">
									<?php if(!empty($qr_chk_exb_dup_name_id_ans_rows ['assoc_nm'])) { ?>
											<h3><strong>Startup Innovation Zone</strong></h3>
									<?php }?>
									<h4 class="block">Enter Exhibitor Details </h4>
									...
									<div class="alert alert-info">Please provide the <strong>Names of the persons who would be manning the stall (EXHIBITOR)</strong> from your organisation
									</div>
...
<script src="js/exhibitor-spoc2.js?lolk3s"></script>
<script>
	...
	function go_back(){
		window.location=('exhibitor_spoc.php?rt=retds4fn324rn_ed24d3it');
	  }
