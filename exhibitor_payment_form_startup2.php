<?php
	...
	if( ($temp_exhi_name == "") || ($temp_addr1 == "") || ($temp_city == "")  || ($temp_state == "")  || ($temp_country == "")  || ($temp_zip == "") || ($temp_email == "") || ($temp_del_title == "") || ($temp_del_fname == "") || ($temp_del_lname == "") || ($temp_del_email == "") || ($temp_del_mob == "")){
		...
		echo "<script language='javascript'>window.location = 'exhibitor_payment_form_startup.php?rt=retds4fn324rn_ed24d3it';</script>";
		exit; 
	}
	...
	mysqli_query($link,"INSERT INTO ".$EVENT_DB_FORM_EXHIBITOR_DIR_PAYMENT_TBL_DEMO. "(sector,subsector,exhibitor_name,reg_id,reg_date,reg_time,user_type)".
			" VALUES('$temp_exhi_sector','$temp_exhi_subsector', '$temp_exhi_name','$reg_id','$temp_reg_date','$temp_reg_time','Startup cell Karnataka') ") or die(mysqli_error($link));
	...
	$tin_no  = "TIN-BTS".$EVENT_YEAR."-EXH-STC-";
	/*if($temp_exhi_sector == 'Bio Technology') {
	...
	('Conference Delegate','$nationality','$curr','Group','$sub_delegates','Complimentary','Complimentary','$amt_ext','$reg_id','$temp_reg_date','$temp_reg_time','$temp_exhi_sector','$tin_no1', '$pin_no', 'Complimentary Exhibitors Delegate','Startup cell Karnataka')")or die(mysqli_error($link));
	...
	if($curr == 'Indian') {
		...
		mysqli_query($link,"UPDATE ".$EVENT_DB_FORM_REG_DEMO." SET cata2 = 'International Premium Delegate' where reg_id = '$reg_id'") or die(mysqli_error($link));
	}
	...
	echo "<script language='javascript'>window.location = 'exhibitor_payment_form_startup3.php';</script>";
	exit;
?>