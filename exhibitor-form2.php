...
$ret = mysqli_escape_string($link,htmlspecialchars(@$_GET['rt']));

$return = mysqli_escape_string($link,htmlspecialchars(@$_GET['ret']));

$assoc_nm = mysqli_escape_string($link,htmlspecialchars(@$_POST['assoc_nm']));

if ($ret == "retds4fn324rn_ed24d3it") {

	$ver_ex = mysqli_escape_string($link,htmlspecialchars(@$_POST['vercode_ex']));

	if ((!isset($ver_ex)) || ($ver_ex == '')) {
	...
	$text = mysqli_escape_string($link,htmlspecialchars(@$_POST['vercode_ex']));

} else {

	$ver_ex = mysqli_escape_string($link,htmlspecialchars(@$_POST['vercode_ex']));

	$ver_ex_s = mysqli_escape_string($link,htmlspecialchars(@$_SESSION['vercode_ex']));

	if (($ver_ex != $ver_ex_s) || ($ver_ex_s == '')) {
...
$temp_booth_area_unit = htmlspecialchars(@$_POST['booth_area_unit'], ENT_QUOTES, 'UTF-8');
...
$temp_cp_title = htmlspecialchars(@$_POST['cp_title'], ENT_QUOTES, 'UTF-8');

$temp_cp_title = trim($temp_cp_title);



$temp_cp_fname = mysqli_real_escape_string($link,htmlspecialchars(@$_POST['cp_fname'], ENT_QUOTES, 'UTF-8'));

$temp_cp_fname = trim($temp_cp_fname);



// $temp_cp_mname = htmlspecialchars(@$_POST['cp_mname'], ENT_QUOTES, 'UTF-8');

// $temp_cp_mname = trim($temp_cp_mname);



$temp_cp_lname = mysqli_real_escape_string($link,htmlspecialchars(@$_POST['cp_lname'], ENT_QUOTES, 'UTF-8'));

$temp_cp_lname = trim($temp_cp_lname);



$temp_desig = htmlspecialchars(@$_POST['desig'], ENT_QUOTES, 'UTF-8');
...
// $temp_fon_area = htmlspecialchars(@$_POST['fon_area'], ENT_QUOTES, 'UTF-8');
...
$temp_fax_cntry = htmlspecialchars(@$_POST['faxCountryCode'], ENT_QUOTES, 'UTF-8');

// $temp_fax_area = htmlspecialchars(@$_POST['fax_area'], ENT_QUOTES, 'UTF-8');

$temp_fax = htmlspecialchars(@$_POST['fax'], ENT_QUOTES, 'UTF-8');

$temp_email = htmlspecialchars(@$_POST['email'], ENT_QUOTES, 'UTF-8');
...
$temp_profile = mysqli_real_escape_string($link,htmlspecialchars(@$_POST['exbhi_profile'], ENT_QUOTES, 'UTF-8'));
...
if ($temp_profile_len < 250) {

	echo "<script language='javascript'>alert('Please Enter Profile more than or equal to 250 characters.');</script>";
...
if ($temp_profile_len > 700) {

	echo "<script language='javascript'>alert('Please Enter Profile less than or equal to 700 characters.');</script>";
...
if (!empty($_FILES["logo"]["name"]) && $_FILES["logo"]["size"] > 0) {
	...
	$target_file = $target_dir . pathinfo(str_replace(' ', '-', $_FILES["logo"]["name"]), PATHINFO_FILENAME) . '-' . date('YmdHis') . '.' . $imageFileType . '.' . $imageFileType;
