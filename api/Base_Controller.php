...
class Base_Controller extends CI_Controller {
	...
	function checkProfileCompleted() {
		...
		$delData = $this->interlinx_reg_model->get_reg_detail_by_criteria($condition);
		
		if(empty($delData['terms_condition'])) {
		  redirect('terms-and-condition');
		}

		if(empty($delData['org_name']) || empty($delData['desig']) || empty($delData['mob_cntry_code']) || empty($delData['mob_number']) || empty($delData['org_profile'])) {
			//echo "<script language='javascript'>alert('Your profile is still incomplete. Please fill complete profile to enabled the full features.');</script>";
			$this->session->set_flashdata('profile_not_completed', true);

			redirect('personal-detail/update');
		} else if(empty($delData['my_industries'])) {
			$this->session->set_flashdata('profile_not_completed', true);
