...
// exit;
	...
	$qr_chk_exb_id_1 = mysqli_query ($link, "SELECT * FROM $EVENT_DB_FORM_EXHIBITOR_DIR_DETAILS_PHASE_2 WHERE (reg_id='$temp_reg_id') " );
	if (mysqli_num_rows ( $qr_chk_exb_id_1 )) {
	    session_destroy ();
	...
	if(!empty($qr_chk_exb_ans)) {
		...
		$values = trim($values, ',');
		$fields = trim($fields, ',');
		//echo "insert  into " . $EVENT_DB_FORM_EXHIBITOR_DIR_DETAILS_PHASE_2 . "($fields) VALUES($values)";
		mysqli_query($link,"insert  into " . $EVENT_DB_FORM_EXHIBITOR_DIR_DETAILS_PHASE_2 . "($fields) VALUES($values)") or die(mysqli_error($link));
		...
		if(mysqli_num_rows($user_data) > 0) {
			while ($row = mysqli_fetch_assoc($user_data)) {
				...
				foreach ($row as $key=>$value) {
					if($key != 'srno') {
						$fields .= $key . ',';
						$values .= "'" . mysqli_real_escape_string($link,htmlspecialchars($value, ENT_QUOTES, 'UTF-8')) . "',";
					}
				}
				$values = trim($values, ',');
				$fields = trim($fields, ',');
				...
				$country_code = '';
				/*$dele_cellno_arr = explode("-", str_replace('+', '', $row['mob']));
				...
				$data['country'] = $qr_chk_exb_ans['country'];
				//Call API
				// sendchkdinapi($data);
	...
	$qr_chk_exb_id = mysqli_query ($link, "SELECT * FROM $EVENT_DB_FORM_EXHIBITOR_DIR_DETAILS_PHASE_2 WHERE (reg_id='$temp_reg_id') " );
	...
	$str_career = "Thank you for submitting Exhibitor Stall Manning details on " . $EVENT_NAME . " " . $EVENT_YEAR;
	...
	/* ---------------------------------------------admin---------------------------------------------------------------- */
	require "exbitor-emailer-admin.php";
	//echo $str_exb;echo $str_exb_admin;//exit;
	...
	$temp_p_email = $EVENT_ENQUIRY_EMAIL;
	$temp_p_name = $EVENT_NAME . " " . $EVENT_YEAR;
	$str_career = "New Exhibitor Stall Manning details on " . $EVENT_NAME . " " . $EVENT_YEAR . " - " . $temp_fascia_name;
	...
	//================================ Save Delegate data ==================================================================
	//##################################################################################################################################
	$reg_id = $qr_chk_exb_ans_1['srno'] . '_' . $_SESSION ['vercode_ex'];
	...
	if(!empty($qr_gt_user_data_ans_row)) {
		...
		for ($i = 1; $i <= $qr_gt_user_data_ans_row['sub_delegates']; $i++) {
			...
			$job_title       = $qr_gt_user_data_ans_row['job_title' . $i];
			...
			$data['mobile']= $phone;
			$data['company']= $qr_gt_user_data_ans_row['org'];
			$data['designation']= $job_title;
			if($qr_gt_user_data_ans_row['cata'] == 'Complimentary Exhibitors Delegate') {
				$data['category_id'] = 1885;
				$data['qsn_366'] = 'Delegate';
			} 
			if($qr_gt_user_data_ans_row['cata'] == 'Complimentary Sponsor Delegate') {
				$data['category_id'] = 1883;
				$data['qsn_366'] = 'Delegate';
			}
			$data['country'] = $qr_gt_user_data_ans_row['country'];
	...
	}//print_r($qr_gt_user_data_ans_row);exit;
	...
	for ($i = 1; $i <= $qr_gt_user_data_ans_row['sub_delegates']; $i++) {
		...
		if ($qry_email_chk_num >= 1) {
			...
			$data['designation']= $job_title;
			if($qr_gt_user_data_ans_row['cata'] == 'Complimentary Exhibitors Delegate') {
				$data['category_id']= 735;
				$data['print_val'] = 'Delegate';
			} else if($qr_gt_user_data_ans_row['cata'] == 'Complimentary Sponsor Delegate') {
				$data['category_id']= 736;
				$data['print_val'] = 'Delegate';
			}
			//$data['booking_id']= $qr_gt_user_data_ans_row['tin_no'];
			//$data['additional_data_1']= $qr_gt_user_data_ans_row['assoc_name'];
			//$data['additional_data_2']= $qr_gt_user_data_ans_row['city'];
			//$data['additional_data_3']= $qr_gt_user_data_ans_row['state'];
			//Call API
			callUniversalAPI($data);*/
...
,
	...
	while ($qr_gt_user_inx_login_data_ans = mysqli_fetch_array($qr_gt_user_inx_login_data_id)) {
		...
		/*mail sent for interlinx is not going to the user */
		...
		//$recipient = array('','test.interlinks@gmail.com','', 'mayuriladi39@gmail.com');
	...
	$recipient = array('','test.interlinks@gmail.com',$EVENT_ENQUIRY_EMAIL, 'chandrachood.as@mmactiv.com');
	...
	elastic_mail("Thank you for Registering with " . $EVENT_NAME . " " . $EVENT_YEAR, $mail_body, $recipients);
	...
	echo "<script language='javascript'>window.location= 'exhibitor6.php?exhi=E34XH3IDf6gyy77&exhibitor_id=" . $exhibitor_id_ex . "';</script>";
	exit;
?>