<?php 
	...
	$EVENT_DB_FORM_SPKR_DATA = $db_speakers_directory_data_tbl = $EVENT_DB_FORM_SKILL_SPKR_DATA;
	$EVENT_DB_FORM_SPKR_PARA = $db_speakers_directory_para_tbl = $EVENT_DB_FORM_SKILL_SPKR_PARA;
	$EVENT_DB_FORM_SPKR_MAP = $db_speakers_directory_mapping_tbl = $EVENT_DB_FORM_SKILL_SPKR_MAP;
	...
	if( ($speaker_title == "") || ($speaker_fname == "")){				
	...
	$speaker_email_1 = $_POST['speaker_email_1'];
	...
	$para_title_add = @$_POST['para_title_'.$i];
	$para_subtitle_add = @$_POST['para_subtitle_'.$i];
	$para_desc_add = @$_POST['para_desc_'.$i];
	$para_desc_add = addslashes($para_desc_add);
	$para_order_no = @$_POST['order_no_para_'.$i];
	$para_element_id = $temp_para_id;
	...
	$data['phone']=$res['speaker_mob'];
	$data['booking_id']=$res['speaker_id'];

	$ticket_id = 31424 ;
	$ticket_type='Speaker';
	//callUniversalAPI($data,$ticket_id,$ticket_type);
	$form_title = true;
	$speker_detail = 'Bengaluru Skill 2019 Speaker Registration Form Data';
	...
	$Subject = "Thank you for Bengaluru Skill 2019 Speaker registration on ". $EVENT_NAME . ' ' . $EVENT_YEAR ;
	$mail->Subject    = $Subject;
	$mail->MsgHTML($mail_body);
	$recipients = array('','test.interlinks@gmail.com', '', 'sandhya.nanjappa@mmactiv.com', 'sjamalan@gmail.com', 'mohanram@mmactiv.com', '', $res['speaker_email_1']);
	...
	//exit;
	echo "<script language='javascript'>window.location = ('skill-speakers-registration3.php');</script>";
	exit;
?>