<?php
	...
	mysqli_query($link,"insert into " . $EVENT_DB_FORM_REG_DEMO . "(org_reg_type,nationality,cata,curr,gr_type,sub_delegates,paymode,pay_status,amt_ext,amt_per_del,selection_amt,total,reg_id,reg_date,reg_time,sp_msg,dollar,assoc_name,member_of_assoc,assoc_srno,user_type,conference_type) values ('$cata_m','$nationality','$cata','$curr','$grp','$total_dele','$paymode','$pay_status','$amt_ext','$rate_org','$amt','$amt','$reg_id','$ddate','$ttime','$temp_sess_sp_msg','$dollar','$assoc_name', '$member_of_assoc','$assoc_srno','Biospectrum Media','$conf_type')") or die(mysqli_error($link));
	...
	//exit;
	echo "<script language='javascript'>window.location = 'registration-del3.php?en=$en&assoc_name=$assoc_name';</script>";
	exit;
?>