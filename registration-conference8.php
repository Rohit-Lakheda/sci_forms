	   ...
	   //$recipients = array('', 'test.interlinks@gmail.com','', 'mayuri.ladi@interlinks.in');
	    ...
	    /* if($_POST['make_payment'] == 1) {
	    	...
	    	if($qr_gt_user_data_ans_row['paymode'] == 'Credit Card' || $qr_gt_user_data_ans_row['paymode'] == 'Debit Card' || $qr_gt_user_data_ans_row['paymode'] == 'i Banking') {
		    	echo "<script language='javascript'>window.location = 'registration-conference9.php';</script>";
		    	exit;
	    	}
	    } */
	}
	
	echo "<script language='javascript'>alert('Sorry, your registration-conference has been failed!');</script>";
	echo "<script language='javascript'>window.location = 'registration-conference.php';</script>";
	exit;
?>