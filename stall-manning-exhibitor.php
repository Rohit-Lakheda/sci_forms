...
if ($ret == "retds4fu324rn_ed24d3it") {
	...
	if ((!isset($_SESSION["vercode_ex"])) || ($_SESSION["vercode_ex"] == '')) {
		...
		echo "<script language='javascript'>document.location=('stall-manning-exhibitor.php');</script>";
		exit;
	}
...
if (isset($_GET['a']) && !empty($_GET['a'])) {
	$assoc_name1 = mysqli_escape_string($link,htmlspecialchars($_GET['a']));
	...
	if (isset($discountDetail['reg_done'])) {
		...
		if ($discountDetail['reg_done'] >= $discountDetail['total_reg']) {
			$active = 0;
			session_destroy();
			echo "<script language='javascript'>alert('For exhibitor " . $discountDetail['assoc_name'] . " registrations seats are full.');</script>";
			echo "<script language='javascript'>window.location = 'stall-manning-exhibitor.php';</script>";
			exit;
		}
	} else {
...
/* $assoc_name = @$_GET['assoc_name'];
 if($assoc_name == 'STPI') {
 echo "<script language='javascript'>window.location = 'stall-manning-exhibitor.php?assoc_name=SOFTWARE TECHNOLOGY PARKS OF INDIA(STPI)';</script>";
 exit;
 } */
?>
	...
	<div class="col-md-12">
		<div class="portlet light bordered" id="registration_form_1">
			...
			<div class="portlet-body form">
				<?php if (date('Y-m-d H:i') <= '2025-11-20 19:00' && $active == "1") { ?>
					<form action="stall-manning-exhibitor2.php<?php echo !empty($ret) ? '?ret=' . $ret : ''; ?>"
								...
								for ($i_exb = 1; $i_exb <= $total_exbhitors; $i_exb++) { ?>
									<div class="h4">Personal Information of Exhibitor:
										...
										<input name="del_category<?php echo $i_exb; ?>" type="hidden"
											id="del_category<?php echo $i_exb; ?>" class="del_category" value="Exhibitor" />
									</div>
									...
									<div class="form-group">
										<label class="col-md-3 control-label">Exhibitor Name (Organisation)
											<?php if ($i_exb <= 2) { ?> <span class="dips-required"> * </span>
											<?php } ?></label>
										<div class="col-md-6">
											<input type="text" class="form-control" name="org<?php echo $i_exb; ?>"
												id="org<?php echo $i_exb; ?>" <?php if ($i_exb <= 2) { ?>required<?php } ?>
												value="<?php echo $_SESSION['org1']; ?>" />
										</div>
									</div>
									<div class="form-group">
										<label class="control-label col-md-3">Contact Person Name<?php if ($i_exb <= 2) { ?>
										...
										<div class="col-md-2"><input type="text" class="form-control" placeholder="First Name"
												name="fname<?php echo $i_exb; ?>" id="fname<?php echo $i_exb; ?>" <?php if ($i_exb <= 2) { ?>required<?php } ?> maxlength="100"
												onkeyup="check_char(event,'fname1');"
												value="<?php echo $_SESSION['fname1']; ?>"></div>
										<div class="col-md-2"><input type="text" class="form-control" placeholder="Last Name"
												name="lname<?php echo $i_exb; ?>" id="lname<?php echo $i_exb; ?>" <?php if ($i_exb <= 2) { ?>required<?php } ?> maxlength="100"
												onkeyup="check_char(event,'lname1');"
												value="<?php echo $_SESSION['lname1']; ?>"></div>
									</div>
									<div class="form-group">
										<label class="col-md-3 control-label">Designation <?php if ($i_exb <= 2) { ?> <span
													class="dips-required"> * </span> <?php } ?></label>
										<div class="col-md-6">
											<input type="text" class="form-control" name="desig<?php echo $i_exb; ?>"
												id="desig<?php echo $i_exb; ?>" maxlength="249" <?php if ($i_exb <= 2) { ?>required<?php } ?> value="<?php echo $_SESSION['desig1']; ?>" />
										</div>
									</div>
									<div class="form-group">
										<label class="col-md-3 control-label">Mobile No <?php if ($i_exb <= 2) { ?> <span
													class="dips-required"> * </span> <?php } ?></label>
										<div class="col-md-6">
											<input type="number" class="form-control" name="mob<?php echo $i_exb; ?>"
												id="mob<?php echo $i_exb; ?>" maxlength="12" <?php if ($i_exb <= 2) { ?>required<?php } ?> value="<?php echo $_SESSION['mob1']; ?>" />
										</div>
									</div>
									...
									<div class="form-group">
										<label class="col-md-3 control-label">Email Address <?php if ($i_exb <= 2) { ?> <span
													class="dips-required"> * </span> <?php } ?></label>
										<div class="col-md-6">
								...
								<div class="form-group">
									...
									<div class="col-md-6">
										<div class="input-group">
											<input name="vercode_ex" type="text" class="form-control" id="vercode_ex"
												maxlength="10" required autocomplete="off" />
											<input name="test" type="hidden" id="test"
												value="<?php echo $_SESSION["vercode_ex"]; ?>" />
											<span class="input-group-addon"
												style="background-image: url('images/verify_img_bg.JPG'); text-align: center; font-size: 32px; padding: 0 15px 1px;"><?php echo $_SESSION["vercode_ex"]; ?></span>
										</div>
									</div>
								</div>
							</div>
...
<script>
	...
	function validate_ex2() {
		...
		for (var j = 1; j <= total_exbhitors; j++) {
			if (document.getElementById("org" + j).value == "") {
				alert("Please fill exhibitor name ");
				document.getElementById("org" + j).focus();
				return false;
