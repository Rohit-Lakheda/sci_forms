...
if (isset($_POST['make_payment'])) {
	...
	if ($qr_gt_user_data_ans_row['assoc_srno'] == 101) {
		$qr_gt_user_data_id1      = mysqli_query($link,"SELECT * FROM " . $EVENT_DB_FORM_EXHIBITOR_ON_SPOT . " WHERE reg_id = '$reg_id'");
	...
	if (!empty($qr_gt_user_data_ans_row)) {
		...
		if ($result['srno'] == 101) {
			mysqli_query($link,"insert  into " . $EVENT_DB_FORM_EXHIBITOR_ON_SPOT . "($fields) VALUES($values)");
		} else {
	...
	for ($i = 1; $i <= $qr_gt_user_data_ans_row['sub_delegates']; $i++) {
		...
		//break;
		//############################################################################
		if ($result['srno'] != 74 && $result['srno'] != 101) { //($qr_gt_user_data_ans_row['cata' . $i] == 'Premium Delegate') || ($qr_gt_user_data_ans_row['cata' . $i] == 'International Premium Delegate') ) {
...
,
	...
	$mail->SetFrom($EVENT_ENQUIRY_EMAIL, $EVENT_NAME . ' ' . $EVENT_YEAR);
	$mail->Subject    = $res['title1'] . " " . $res['fname1'] . " " . $res['lname1'] . " Thank you for Registering with " . $EVENT_NAME . " " . $EVENT_YEAR;
	$mail->MsgHTML($mail_body);
	...
	$Subject    = $res['title1'] . " " . $res['fname1'] . " " . $res['lname1'] . ", Thank you for Registering with " . $EVENT_NAME . " " . $EVENT_YEAR;
	...
	echo "<script language='javascript'>window.location = 'registration-assoc-conf-vip9.php?a=$assoc_code&id=" . $qr_gt_user_data_ans_row['tin_no'] . "';</script>";
	mysqli_close($link);
	...
	if (($qr_gt_user_data_ans_row['paymode'] == 'Credit Card' || $qr_gt_user_data_ans_row['paymode'] == 'Debit Card' || $qr_gt_user_data_ans_row['paymode'] == 'i Banking' || $qr_gt_user_data_ans_row['paymode'] == 'Google pay') && $qr_gt_user_data_ans_row['pay_status'] == 'Not Paid') {
		/*//echo "<script language='javascript'>window.location = 'registration-assoc-conf-vip9.php?id=" . $qr_gt_user_data_ans_row['tin_no'] . "';</script>";*/
		session_destroy();
	...
	/* if($_POST['make_payment'] == 1) {
	    	...
	    	if($qr_gt_user_data_ans_row['paymode'] == 'Credit Card' || $qr_gt_user_data_ans_row['paymode'] == 'Debit Card' || $qr_gt_user_data_ans_row['paymode'] == 'i Banking') {
		    	echo "<script language='javascript'>window.location = 'registration-assoc-conf-vip9.php';</script>";
		    	exit;
	    	}
	    } */
}
